const Prospecto = require('../../models/prospect.model');
...
const generatePDF = async (candidatos) => {
                ...
                table: {
                    ...
                    body: [
                        ...
                        ...candidatos.map(candidato => [
                            { text: candidato.nombre, bold: true, color: 'red' }, // Estilo para el título en negrita y rojo
                            candidato.apellido,
                            '',
                            '' 
                          ])
...
class rptCandidatosPorEmpresaService {
    static async crear(Id) {
        try {
            const prospectos = await Prospecto.obtenerProspectosTodosPorEmpresa(Id);
            generatePDF(prospectos);
        } catch (error) {
            throw new Error('Error al crear el DPF, servicio: ' + error);
        }
    }
} module.exports = rptCandidatosPorEmpresaService;